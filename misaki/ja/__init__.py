    ...
    def __call__(self, text) -> Tuple[str, Optional[List[MToken]]]:
        ...
        for word in pyopenjtalk.run_frontend(text):
            ...
            if mora_size > 0:
                ...
                assert (
                    len(moras) == mora_size
                    or len(moras) + (1 if moras[0] == "ー" else 0) == mora_size
                ), (moras, mora_size)
            chain_flag = (
                mora_size > 0
                and tokens
                and tokens[-1]._.mora_size > 0
        ...
        for tk in tokens:
            ...
            if (
                tk._.mora_size
                and not tk._.chain_flag
                and result
                and result[-1] in TAILS
                and not tk._.moras[0] == "ン"
            ):
                result += " "
                pitch += "j"
            result += tk.phonemes + tk.whitespace
            pitch += (
                ("j" * len(tk.phonemes)) if tk._.pitch is None else tk._.pitch
            ) + "j" * len(tk.whitespace)
