...
class Lexicon:
    ...
    def __call__(self, tk, ctx):
        word = (
            (tk.text if tk._.alias is None else tk._.alias)
            .replace(chr(8216), "'")
            .replace(chr(8217), "'")
        )
        ...
        if ps is not None:
            return (
                apply_stress(self.append_currency(ps, tk._.currency), tk._.stress),
                rating,
            )
...
class G2P:
    ...
    def fold_left(self, tokens: List[MToken]) -> List[MToken]:
        ...
        for tk in tokens:
            tk = (
                merge_tokens([result.pop(), tk], unk=self.unk)
                if result and not tk._.is_head
                else tk
            )
            result.append(tk)
        return result
    ...
    def resolve_tokens(tokens):
        ...
        prespace = (
            ...
            or len(
                {
                    0 if c.isalpha() else (1 if is_digit(c) else 2)
                    for c in text
    ...
    def __call__(self, text: str, preprocess=True) -> Tuple[str, List[MToken]]:
        ...
        for i, w in reversed(list(enumerate(tokens))):
            ...
            while left < right:
                if any(
                    tk._.alias is not None or tk.phonemes is not None
