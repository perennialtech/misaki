...
from ._tokenization import (SUBTOKEN_JUNKS, preprocess, retokenize,
                            tokenize)
...
class G2P:
    ...
    def __call__(self, text: str, preprocess_fn=True) -> Tuple[str, List[MToken]]:
        ...
        for i, group in reversed(list(enumerate(token_groups))):
            w = list(group.tokens)
            if len(w) == 1:
                tk = w[0]
                if tk.phonemes is None:
        ...
        for group in token_groups:
            if len(group.tokens) > 1:
                final_tokens.append(merge_tokens(list(group.tokens), unk=self.unk))
            else:
